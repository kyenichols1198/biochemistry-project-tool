# -----------------------------------------------------------
# demonstrates how to expound the bufurcation diagram at
# different rates of protein decay that may happen in vitro
...
def tester():
    ...
    gammas = [1,1.1]
    gamma = gammas[0]
    while gamma <= gammas[1]:
        beta=1773.574498*(1/gamma)
        paramk=2952.672305
        araConv=0.036299
        atcConv=.194
        step=.01
        ...
        plt.plot(np.array(line_l), np.array(atcRange), label='gamma (L)= '+str(gamma))
        plt.plot(np.array(line_r), np.array(atcRange), label='gamma (R)= '+str(gamma))
